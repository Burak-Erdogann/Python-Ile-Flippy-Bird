...
class Kus(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if oyun_bitti == False:
            ...
            self.image = pygame.transform.rotate(self.images[self.index], self.vel * -4) #kusun yonunu degistirir
        else :
...
class Kalkan(pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load('kalkan.png')  # Kalkan resmini yükle
        self.rect = self.image.get_rect()
...
def kalkan_olustur():
    while True:
        kalkan = Kalkan()  # Kalkan nesnesi oluştur
        if not pygame.sprite.spritecollideany(kalkan, pipe_group):
            kalkan_group.add(kalkan)  # Kalkan nesnesini gruba ekle
            break  # Çakışma yoksa döngüden çık
class Altin_elma(pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load('altın_elma.png')  # Kalp resmini yükle
        self.rect = self.image.get_rect()
        self.rect.x =random.randint(50, ekran_genislik - 50)
        self.rect.y = random.randint(50, ekran_yukseklik - 250)
        if self.rect.y > ekran_yukseklik - 150:
...
class Yatay_ok(pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load('ok2.png')  # Kalp resmini yükle
        self.rect = self.image.get_rect()
        self.rect.x =random.randint(50, ekran_genislik - 50)
        self.rect.y = random.randint(50, ekran_yukseklik - 250)
        if self.rect.y > ekran_yukseklik - 150:
...
def ok_olustur():
    while True:
        ok = Yatay_ok()  # Kalp nesnesi oluştur
        if not pygame.sprite.spritecollideany(ok, pipe_group):

            ok_group.add(ok)  # Kalp nesnesini gruba ekle
            break  # Çakışma yoksa döngüden çık

class Simsek(pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load('simsek2.png')  # Kalp resmini yükle
        self.rect = self.image.get_rect()
        self.rect.x =random.randint(50, ekran_genislik - 50)
        self.rect.y = random.randint(50, ekran_yukseklik - 250)
        if self.rect.y > ekran_yukseklik - 150:
...
def simsek_olustur():
    while True:
        simsek = Simsek()  # Kalp nesnesi oluştur
        if not pygame.sprite.spritecollideany(simsek, pipe_group):
            simsek_group.add(simsek)  # Kalp nesnesini gruba ekle
            break  # Çakışma yoksa döngüden çık
...
pipe_gap = 150 # boru 150 piksel o yüzden yarısını atıyoruz
pipe_frekans = 1500
...
ok_frekans = random.randint(10000,20000)
...
while run:
    ...
    if len(pipe_group) > 0: #scoreu arttırma ve kontrol
    ...
    if flappy.rect.bottom >= 768: #zemine carpma
        oyun_bitti= True
        ucus = False
        if not olum_ses_calindi:
            olum_sesi.play()
    ...
    if oyun_bitti == False and ucus == True:
        ...
        if time_now - son_pipe > pipe_frekans:
            ...
            toppipe = Pipe(ekran_genislik, int(ekran_yukseklik / 2) + pipe_yukseklik - 75, 1)
            pipe_group.add(btmpipe)
            pipe_group.add(toppipe)
            son_pipe = time_now

        yer_hareketi -= hareket_hizi
        if abs(yer_hareketi) > 35:  #35 piksel oldugu icin bitince basa sarmasi icin yapildi
            yer_hareketi = 0
    ...
    for i in pygame.event.get():
        if i.type == pygame.QUIT:
            run = False
            calisiyor = False
        if i.type == pygame.K_SPACE and  ucus == False and oyun_bitti == False:
            ucus = True
